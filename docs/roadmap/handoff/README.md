...
- **The `prism-setup` Claude skill does not exist.** `PROGRESS.md` claims it was
  created; `.claude/skills/` contains only classify, extract-entities, reconcile,
  schema-bridge, wikilinks. The current plugin (`.claude-plugin/plugin.json`)
  bundles those + the Parachute MCP only. The "set up your Prism" capability is
  essentially unbuilt — see PLUGIN-SETUP-HANDOFF §1.
- **`prism-setup.ts` only provisions the server `.env` + tag seeding** — nothing
  for the vault itself, the desktop config, Matrix, Google, or any integration.
- **Federation transport is built but never reaches the client.** The peer
  ...
  **never called** (no peer-URL registry) and both clients connect to `/collab`
  ...
  blocking BUILD work — see FEDERATION-TWO-HUB-HANDOFF §0 and §3.
- **`verify-gateway.ts` hardcodes the owner's note IDs** (lines 13–15:
  `TAG`/`GRANTED_NOTE`/`FORBIDDEN_NOTE`). It must be generalized to
  self-provision throwaway notes before it runs on any vault — see
  TESTING-HANDOFF §4.1.
