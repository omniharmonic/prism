...
Prism can run as a governed, geospatial wiki for a bioregion. Status, verification battery, and the readiness audit live in `docs/roadmap/bioregional-commons-PROGRESS.md`; the two plans are `bioregional-commons-1-governance.md` / `-2-graph.md`.

**Governance is note-native and a parallel subsystem.** State (config, roles, policies, memberships, proposals, votes, revisions, audit) is stored as `governance-*` tagged vault notes. Layers: pure engine `apps/server/src/governance.ts` → store seam `governance-store.ts` (notes ⇄ structures) → service `governance-service.ts` (`mutateGovernance` is the single choke point) → routes `routes/governance.ts` (`/api/governance`, mounted before the gateway). UI is `packages/core/.../network/governance/GovernancePanel.tsx`, surfaced as **Network → Governance** (the standalone `/governance` route is dev-only).
- **The bootstrap lock is a one-way latch.** Before *Enable & lock*, only the bootstrap owner may configure directly; after, `canMutateGovernanceDirectly` is false for everyone — including the owner — and every change (incl. disabling) needs an approved `amend_governance` proposal. Do not add a bypass. Recovery from a bricked constitution = delete the `governance-config` note with the owner Bearer over loopback.
- **Governance does NOT gate content.** `governance*.ts` is imported only by its own route; `routes/api.ts`, `collab.ts`, and `permissions.ts` never consult it. Governance *roles/powers* and content *ACL levels* (`view<comment<suggest<edit<own`) are two disconnected systems. A governance role grants zero editing rights.
...
- **Known bugs (see PROGRESS audit):** `effect()` in `governance-service.ts` has no dedupe, so re-running `commons-init` duplicates every role/policy/member; nothing can remove/revoke a member/role/policy in UI or API; the lock can be entered with no eligible voter.
